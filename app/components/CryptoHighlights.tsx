...
export default function CryptoHighlights() {
  ...
  const options = {
    method: 'GET',
    headers: {accept: 'application/json', 'x-cg-demo-api-key': 'CG-2dos2tsQyepiGzFCzGJXzJyG'}
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const marketCapUsd = globalRes.data.data.total_market_cap.usd
      const volumeUsd = globalRes.data.data.total_volume.usd
      const change = globalRes.data.data.market_cap_change_percentage_24h_usd
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-6">
      ...
      <p className="text-gray-500 text-sm mb-6">
        The global cryptocurrency market cap today is ${(+marketCap / 1e12).toFixed(2)} Trillion,
        a <span className={marketCapChange < 0 ? 'text-red-500' : 'text-green-500'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className=" flex flex-col items-between justify-between rounded-xl border-2  border-gray-300 p-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-blue-500 text-sm cursor-pointer">View more &gt;</span>
          </div>
          {trending.slice(0, 3).map((coin) => (
            <div key={coin.symbol} className="flex justify-between items-center py-1">
